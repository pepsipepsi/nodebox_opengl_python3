...
class Color(list):
    ...
    def blend(self, clr, t=0.5, colorspace=RGB):
        ...
        r, g, b = [lerp(a, b, t) for a, b in ch]
        a = lerp(self.a, len(clr)==4 and clr[3] or 1, t)
...
def transform(mode=None):
    if mode == CENTER:
        raise Exception("no center-mode transform")
    return CORNER
...
class BezierPath(list):
    ...
    def arcto(self, x, y, radius=1, clockwise=True, short=False):
        ...
        phi = angle(x0,y0,x,y)
        for p in graphics.arcto(x0, y0, radius, radius, phi, short, not clockwise, x, y):
    ...
    def arc(self, x, y, width, height, start=0, stop=90):
        ...
        """
        w, h = width*0.5, height*0.5
        for i, p in enumerate(graphics.arc(x-w, y-h, x+w, y+h, start, stop)):
            if i == 0:
                self.moveto(*p[:2])
            self.curveto(*p[2:])
    ...
    def angle(self, t):
        ...
        return angle(pt0.x, pt0.y, pt1.x, pt1.y)
    ...
    def contains(self, x, y, precision=100):
        ...
        if bx <= x <= bx+bw and \
           by <= y <= by+bh:
                ...
                return point_in_polygon(self._polygon[0], x, y)
        return False
...
def findpath(points, curvature=1.0):
    ...
    """
    return findpath(list(points), curvature)
...
class BezierEditor:
    ...
    def rotate(self, pt, angle, handle=BOTH):
        ...
        if handle == BOTH or handle == IN:
            pt1.ctrl2.x, pt1.ctrl2.y = rotate(pt1.ctrl2.x, pt1.ctrl2.y, pt1.x, pt1.y, angle)
        if handle == BOTH or handle == OUT and pt2 is not None and pt2.cmd == CURVETO:
            pt2.ctrl1.x, pt2.ctrl1.y = rotate(pt2.ctrl1.x, pt2.ctrl1.y, pt1.x, pt1.y, angle)
    ...
    def scale(self, pt, v, handle=BOTH):
        ...
        pt1, pt2 = pt, self._nextpoint(pt)
        if handle == BOTH or handle == IN:
            pt1.ctrl2.x, pt1.ctrl2.y = graphics.linepoint(v, pt1.x, pt1.y, pt1.ctrl2.x, pt1.ctrl2.y)
        if handle == BOTH or handle == OUT and pt2 is not None and pt2.cmd == CURVETO:
            pt2.ctrl1.x, pt2.ctrl1.y = graphics.linepoint(v, pt1.x, pt1.y, pt2.ctrl1.x, pt2.ctrl1.y)
    ...
    def smooth(self, pt, mode=None, handle=BOTH):
        ...
        if pt1.cmd == pt2.cmd == CURVETO:
            if mode == EQUIDISTANT:
                d1 = d2 = 0.5 * (
                     distance(pt1.x, pt1.y, pt1.ctrl2.x, pt1.ctrl2.y) + \
                     distance(pt1.x, pt1.y, pt2.ctrl1.x, pt2.ctrl1.y))
            else:
                d1 = distance(pt1.x, pt1.y, pt1.ctrl2.x, pt1.ctrl2.y)
                d2 = distance(pt1.x, pt1.y, pt2.ctrl1.x, pt2.ctrl1.y)
            if handle == IN: 
                a = angle(pt1.x, pt1.y, pt1.ctrl2.x, pt1.ctrl2.y)
            if handle == OUT: 
                a = angle(pt2.ctrl1.x, pt2.ctrl1.y, pt1.x, pt1.y)
            if handle == BOTH: 
                a = angle(pt2.ctrl1.x, pt2.ctrl1.y, pt1.ctrl2.x, pt1.ctrl2.y)
            pt1.ctrl2.x, pt1.ctrl2.y = coordinates(pt1.x, pt1.y, d1, a)
            pt2.ctrl1.x, pt2.ctrl1.y = coordinates(pt1.x, pt1.y, d2, a-180)
        elif pt1.cmd == CURVETO and pt2.cmd == LINETO:
            d = mode == EQUIDISTANT and \
                distance(pt1.x, pt1.y, pt2.x, pt2.y) or \
                distance(pt1.x, pt1.y, pt1.ctrl2.x, pt1.ctrl2.y)
            a = angle(pt1.x, pt1.y, pt2.x, pt2.y)
            pt1.ctrl2.x, pt1.ctrl2.y = coordinates(pt1.x, pt1.y, d, a-180)
        elif pt1.cmd == LINETO and pt2.cmd == CURVETO and i > 0:
            d = mode == EQUIDISTANT and \
                distance(pt1.x, pt1.y, self.path[i-1].x, self.path[i-1].y) or \
                distance(pt1.x, pt1.y, pt2.ctrl1.x, pt2.ctrl1.y)
            a = angle(self.path[i-1].x, self.path[i-1].y, pt1.x, pt1.y)
            pt2.ctrl1.x, pt2.ctrl1.y = coordinates(pt1.x, pt1.y, d, a)
...
def supershape(x, y, width, height, m, n1, n2, n3, points=100, percentage=1.0, range=2*pi, **kwargs):
    ...
    for i in range(points):
        if i <= points * percentage: 
            dx, dy = superformula(m, n1, n2, n3, i*range/points)
...
def texture(img, data=None):
    ...
    raise Exception("unknown image type: %s" % repr(img.__class__))
...
def cache(id, texture):
    ...
    if not isinstance(texture, pyglet.image.Texture):
        raise Exception("can only cache texture, not %s" % repr(texture.__class__.__name__))
...
# jf - this may need to be changed from the callsite, self is not expected -- all of the other methods have it!

    def adjust(self, r=1.0, g=1.0, b=1.0, a=1.0):
        """ Adjusts the image color by multiplying R,G,B,A channels with the given values.
        """
        self.color[0] *= r
        self.color[1] *= g
...
def label(str="", width=None, height=None, **kwargs):
    ...
    label.begin_update()
    label.document = FormattedDocument(str or " ")
    label.width     = width    
    label.height    = height
...
class Text(object):
    ...
    def __getattr__(self, k):
        if k in self.__dict__:
            ...
            if self._label.width != INFINITE: return self._label.width
            ...
            raise Exception("'Text' object has no attribute '%s'" % k)
    ...
    def __setattr__(self, k, v):
        ...
        if k in ("text", "height", "bold", "italic"):
            ...
            self._label.width = v is None and INFINITE or v
        elif k in ("font", "fontname"):
            ...
            self._align = v
            self._label.set_style(k, self._label.width == INFINITE and LEFT or v)
        elif k == "fill":
            self._fill = v 
...
def textpath(string, x=0, y=0, **kwargs):
    ...
    for ch in string:
        try: glyph = glyphs[fontname][w][ch]
            ...
            raise Exception("no glyph path information for %s %s '%s'" % (w, fontname, ch))
        for pt in glyph:
            if pt[0] == MOVETO:
...
class Prototype(object):
    ...
    def _deepcopy(self, value):
        if isinstance(value, FunctionType):
            ...
            raise Exception("Prototype can't bind %s." % str(value.__class__))
    ...
    def update(self):
        ...
        if TIME >= self._t1 or self._vi is None:
            ...
            if self._interpolation == LINEAR:
                ...
                self._vi = self._v0 + (self._v1-self._v0) * smoothstep(0.0, 1.0, t)
            return False
...
class LayerClippingMask(ClippingMask):
    def __init__(self, layer):
        self.layer = layer
    def draw(self, fill=(0,0,0,1), stroke=None):
        w = not self.layer.width  and INFINITE or self.layer.width
        h = not self.layer.height and INFINITE or self.layer.height
...
class Layer(list, Prototype, EventHandler):
    ...
    def __getattr__(self, key):
        ...
        for layer in self:
            if layer.name == key: 
                return layer
        raise Exception("%s instance has no attribute '%s'" % (self.__class__.__name__, key))
    ...
    def render(self):
        ...
        """
        b = self.bounds
        if INFINITE in (b.x, b.y, b.width, b.height):
            raise Exception("can't render layer of infinite size")

#the following line invokes psyco, and antique compiler no longer supported
    ...
    def _bounds(self, local=True):
        ...
        """
        w = self._width.current; w = w is None and INFINITE or w
        h = self._height.current; h = h is None and INFINITE or h
...
#psyco is an ancient compiler no longer supported - there may be a possible substitute
        # if psyco_stopped:
        #     psyco.profile()
        return s
